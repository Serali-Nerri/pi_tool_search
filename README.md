...
npm 来源与显式路径加载会规范化到同一提供者身份；相对路径以会话 cwd 为准，已有符号链接解析为真实路径。自动发现的本地扩展也按真实文件路径保存为 `file:<路径>`，不再共用 `auto`；自动发现的 npm 扩展仍使用 `npm:<包名>`。配置不会使缺少提供者或未在白名单中的工具凭空可用。

旧 `auto` 激活记录不自动归属到某个本地提供者，需重新加载工具；旧 `source: "auto"` 的 `always/deferred` 策略需重新配置。为避免升级后意外启用工具，旧 `excluded` 仅对自动发现的本地提供者保留，直到明确的路径策略覆盖或手动移除旧记录。
