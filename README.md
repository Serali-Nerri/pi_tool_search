...
- 每个会话独立维护 loaded 状态。恢复时优先使用成功加载结果中的提供者身份；策略读写使用 `ctx.cwd`，适配 child cwd 和 worktree。
...
npm 来源与显式路径加载会规范化到同一提供者身份，策略保存使用规范化身份。全局策略不能使缺少提供者扩展、或不在子会话白名单内的工具凭空可用。
...
`config` 默认写全局 `~/.pi/agent/pi-tool-search.json`；`config project` 写当前受信任项目的 `.pi/pi-tool-search.json`（要求项目已信任）。两者打开同一个配置面板，只有保存目标不同。
...
- `verify`：严格 TypeScript 检查及 81 项单元/集成测试，包含真实 Pi SDK 的启动顺序、reload、会话树分支切换和启动后注册工具的状态恢复、snippet 捕获与指南迁移的预算/回退断言、锁定集合与命名默认值、配置面板排序与全局/项目保存。
